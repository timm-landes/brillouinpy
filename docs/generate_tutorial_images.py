...
if __name__ == '__main__':
    ...
    # 9. Fitting
    # -----------------------------------------------------------------------
    dho_fit = bp.analysis.fit.DHO(expected_peaks=1, p0=[0.005, 8.5, 1, 0, 0], bounds=None)
    ...
    irf_plain, _ = bp.analysis.fit.DHO(expected_peaks=1, p0=irf_p0, bounds=irf_bounds).apply(irf_prepared)
    irf_param, _ = bp.analysis.fit.DHO(expected_peaks=1, p0=irf_p0, bounds=irf_bounds,
                                            irf=('gaussian', irf_info['irf_fwhm'])).apply(irf_prepared)
    irf_auto, _ = bp.analysis.fit.DHO(expected_peaks=1, p0=irf_p0, bounds=irf_bounds,
